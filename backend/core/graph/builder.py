...
from core.graph.extractors import extract_advanced_metadata
...
def build_dependency_graph(file_list: list[dict]) -> nx.DiGraph:
    ...
    for file_info in file_list:
        ...
        advanced_meta = extract_advanced_metadata(source_bytes, language)
        G.nodes[rel_path].update(advanced_meta)
...
def get_architecture_context(
    ...
    return {
        ...
        "classes": list(set(all_classes))[:100],
        "functions": list(set(all_functions))[:100],
        "calls": list(set(all_calls))[:50],
        "api_routes": list(set(all_routes))[:50],
        "database_models": list(set(all_models))[:50],
