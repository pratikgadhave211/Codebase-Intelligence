...
# Colour scheme by structural role (RGB)
ROLE_COLOURS = {
    "main":   "#EF4444", # Red (Entry points, high level)
    "medium": "#10B981", # Green (Intermediaries)
    "sub":    "#3B82F6", # Blue (Leaf nodes, pure utilities)
}

# Colour for nodes with circular dependencies (highlights problem files)
CYCLE_COLOUR = "#EAB308" # Yellow (Changed from red since red is now 'main')
...
def render_graph_html(G: nx.DiGraph) -> str:
    ...
    for node, attrs in G.nodes(data=True):
        ...
        if in_deg == 0:
            role = "main"
            role_desc = "Main Component (Entry)"
        elif out_deg == 0:
            role = "sub"
            role_desc = "Sub Component (Leaf)"
        else:
            role = "medium"
            role_desc = "Medium Component (Intermediary)"

        # Use just the filename as the label (keeps graph readable)
        # Full path in the hover tooltip
        label = os.path.basename(node)

        # Apply colour based on role or cycle
    ...
    legend_html = """
    <div style="position: absolute; top: 20px; left: 20px; background: rgba(30, 41, 59, 0.9); padding: 15px; border-radius: 8px; border: 2px solid #334155; color: #f8fafc; font-family: sans-serif; z-index: 1000; box-shadow: 4px 4px 0 #0f172a;">
        <h3 style="margin: 0 0 10px 0; font-size: 14px;">Component Roles</h3>
        <div style="display: flex; align-items: center; margin-bottom: 5px;">
            <div style="width: 12px; height: 12px; border-radius: 50%; background: #EF4444; margin-right: 8px;"></div>
            <span style="font-size: 12px;">Main (Entry point)</span>
        </div>
        <div style="display: flex; align-items: center; margin-bottom: 5px;">
            <div style="width: 12px; height: 12px; border-radius: 50%; background: #10B981; margin-right: 8px;"></div>
            <span style="font-size: 12px;">Medium (Intermediary)</span>
        </div>
        <div style="display: flex; align-items: center; margin-bottom: 5px;">
            <div style="width: 12px; height: 12px; border-radius: 50%; background: #3B82F6; margin-right: 8px;"></div>
            <span style="font-size: 12px;">Sub (Leaf)</span>
        </div>
        <div style="display: flex; align-items: center;">
            <div style="width: 12px; height: 12px; border-radius: 50%; background: #EAB308; margin-right: 8px;"></div>
            <span style="font-size: 12px;">Circular Dependency</span>
        </div>
    </div>
    </body>
    """
